...
import com.nick.lentagifv.RemoteFeedAdapter;
...
public class ExoPlayerRecyclerView extends RecyclerView {
  ...
  public void playVideo(boolean isEndOfList) {
    ...
    if (!isEndOfList) {
      ...
      targetPosition = this.getChildCount() - 1;
    }
    ...
    RemoteFeedAdapter.FeedItemViewHolder holder = ( RemoteFeedAdapter.FeedItemViewHolder) child.getTag();
    if (holder == null) {
      playPosition = -1;
      return;
    }
    mediaCoverImage = holder.getThumbnail();
    viewHolderParent = holder.itemView;
    mediaContainer = holder.getMediaContentView();
    ...
    FeedResponse.Result.Item item = holder.getItem();

    if (item == null) return;
    if (item.getData() == null) return;

    FeedResponse.Result.Item.Data.Block.Data.Video itemVideo = holder.getItem().getData().getYouTube();
    if (itemVideo != null){

      return;
      //todo load youtube video
    }

    if (item.getData().getMedia() == null) return;

    String mediaUUID = item.getData().getMedia().getUuid();
    if (mediaUUID != null) {
      MediaSource videoSource = new ExtractorMediaSource.Factory(dataSourceFactory)
          .createMediaSource(Uri.parse( "https://leonardo.osnova.io/"+ mediaUUID +"/-/format/mp4/"));
